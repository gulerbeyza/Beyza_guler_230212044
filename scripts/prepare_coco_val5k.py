...
YAML_OUT = DROOT / "coco_val5k_beyza.yaml"
...
def _download(url: str, dest: Path) -> None:
    ...
    req = Request(
        url,
        headers={"User-Agent": "BeyzaPrepareCOCO/1.0 (compatible; educational use)"},
    )
    print(f"Downloading\n  {url}\n  -> {dest}")
...
def _write_data_yaml() -> Path:
    ...
    for line in text[:cut].splitlines():
        if line.startswith("path:"):
            lines_out.append(f"path: {DROOT.as_posix()}  # Beyza COCO val subset root")

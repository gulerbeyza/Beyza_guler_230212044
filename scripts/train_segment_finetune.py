...
and writes datasets/coco128_beyza.yaml with an absolute path so training does not
...
DATA_YAML = ROOT / "datasets" / "coco128_beyza.yaml"
...
def _download(url: str, dest: Path) -> None:
    ...
    req = Request(
        url,
        headers={"User-Agent": "BeyzaTrain/1.0 (compatible; educational use)"},
    )
...
def write_data_yaml() -> Path:
    ...
    for line in text.splitlines():
        if line.startswith("path:"):
            out_lines.append(f"path: {COCO_ROOT.as_posix()}  # Beyza local root")
...
def resolve_data_arg(data: str | None) -> str:
    if data and data.lower() not in {"local", "beyza", "project"}:
        return data
    ensure_coco128_seg()
...
def main() -> int:
    ...
    parser.add_argument(
        "--data",
        default="local",
        help="Dataset: 'local' or 'beyza'/'project' (coco128 in ./datasets), or path / ultralytics yaml name.",
    )
